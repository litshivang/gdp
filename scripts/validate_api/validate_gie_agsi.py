"""
Validation script for GIE AGSI API migration (orchestrator + GieAgsiAdapter).

Run from project root:
    python -m scripts.validate_api.validate_gie_agsi
  or
    python scripts/validate_api/validate_gie_agsi.py

Optional env: COUNTRY. Validates: raw_events (AGSI), energy.daily, meta.series.
"""
...
def main():
    ...
    print("=" * 60)
    print("GIE AGSI migration validation")
    print("=" * 60)
    print(f"country={country}")
    print()

    raw_before, daily_before = get_counts()
    print(f"Before: raw_events={raw_before}, energy.daily(AGSI)={daily_before}")
    print()

    try:
        orch = Orchestrator(registry)
        orch.run("AGSI", country=country)
    ...
    print(f"After: raw_events={raw_after}, energy.daily(AGSI)={daily_after}")
    print()
